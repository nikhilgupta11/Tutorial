        ...
        if self.head is None:
            ...
            if location == 0:
                if self.head == self.tail:
                    ...
                    # replce the referance value of first node from head with second node(self.head.next)
                ...
                if self.head == self.tail:
                    ...
                    # setting last nodes value to null
                    node.next = None

                    # connecting new last node to tail
                    self.tail = node
                ...
                # finding current nodes next refrence
                nextNode = tempNode.next

                # connecting current node with nextnode's next refrance
                tempNode.next = nextNode.next

    # deletion of complete Linked List
    def deletionCompleteSLL(self):
        if self.head is None:
            print("Singaly Linked List does not exists")
        else:
            self.head = None
            self.tail = None
...
# deletionSLL method calling
singlyLinkedList.deletionSLL(3)
print([node.value for node in singlyLinkedList])


# deletionCompleteSLL call
singlyLinkedList.deletionCompleteSLL()
print([node.value for node in singlyLinkedList])
